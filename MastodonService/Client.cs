...
namespace MastodonService
{
    public class MastodonClient
    {
        ...
        public async Task<bool> PostImageAsync(string status, Models.ImageUpload image)
        {
            ...
            var content = new StringContent($"{{\"status\":\"{status}\",\"media_ids\":[{imageId}]}}", Encoding.UTF8, "application/json");
        ...
        public async Task<bool> PostMultipleImagesAsync(string status, Models.ImagesList Images)
{
    ...
    var content = new StringContent($"{{\"status\":\"{status}\",\"media_ids\":[{mediaIdsString}]}}", Encoding.UTF8, "application/json");
